...
#       installed and install then in our favourite flavour of Debian.
#
...
sub print_usage() {
	...
	print("\n");
	print("\n");
	print(NAME . " Copyright (C) 2003, 2004 Miquel Oliete <miqueloliete\@softhome.net>\n");
	print("\n");
	...
	print("Usage \n");
	print("\n");
	print("  " . NAME . " [-d|--debug] --option=[build|download] --sources_list=filename  \n");
	print("\n");
...
sub download_packages() {
	...
	for($i=0;$i<scalar(@sources_list_content_lines);$i++) {
		if ($sources_list_content_lines[$i] =~ /^\'(http|ftp):\/\/([^\/]*)(\/[^ ]*\/)([^ \/]+\.deb)\' ([^ ]*) ([^ ]*) ([^ ]*)/) {
			...
			debug_print("\$3-->$3\n");
			debug_print("\$4-->$4\n");
			$target_directory = substr($3, 1, length($3)-1);
			if (! -d $pwd.$3) {
