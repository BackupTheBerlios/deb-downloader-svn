...
sub print_usage() {
	...
	print("Usage : \n");
	print("\n");
	print("  " . NAME . " --help | --version | --option=build|download|install-packages [--dd-root=directory] --file=filename [--skip-update] [--mirror-format] [--execute-dist-upgrade]  [-d|--debug]  \n");
	print("\n");
...
sub copy_downloaded_files {
	...
	for($i=0;$i<scalar(@files);$i++) {
		debug_print("torpedorrrr 1\n");
		if ($files[$i] =~ /^\'(?:http|ftp):\/\/[^\/]*\/([^ ]*\/)([^ \/]+.*)' [^ ]* [^ ]* [^ ]*/) {
			debug_print("file--->$deb_downloader_options{'dd-root'}$1$2\n");
			debug_print("path--->$target_directory$2\n");
			copy("$deb_downloader_options{'dd-root'}$1$2", "$target_directory$2");
		}
	}
	close(SOURCE_DIR);
