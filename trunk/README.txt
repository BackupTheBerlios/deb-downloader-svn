...
~~~~~~~~~~
	...
	This is an utility written by me called deb-downloader, a tool implemented for an specific GNU/Linux distro 
called Debian (or Debian based distros). deb-downloader was written two month ago (06-08-2004) , when my "high speed" 
internet connection was down because of my house moving. This event made think me about which was the only way for 
upgrading my Sid (Debian has three versions (stable, testing and sid also called Sid (an urban legend says it means 
System In Development)).

	I had heard about a Debian tool called apt-zip, but I didn't have a GNU/Linux distro at work so I decide to 
write this tool. deb-downloader is written in Perl, so the only requirement for its use is having an operating system 
with a perl interpreter as *nix systems, Macintosh (OS 7-9 and X), VMS, Windows, etc. (see http://www.cpan.org/ports/index.html 
for more details).
...
~~~~~~~~~~~~~~
	...
	There are two requirements for using deb-downloader.
		...
		- Debian GNU/Linux (or Debian based distro) with perl interpreter for deb-downloader-build and deb-downloader-install.
		- Perl interpreter with Net::FTP and Net::HTTP modules for deb-downloader-download.
...
~~~~~
	...
	This script is fisically divided in three parts. The first and the third one are executed in a Debian distro and the 
second one in a machine with any operating system containing a perl interpreter.
	...
	- deb-downloader-download : This is used for download the .deb packages needed. It can be executed in any computer 
		...
		deb-downloader-download --file=filename1...filenamen [-d|--debug] [--dd-root=directory] [--skip-downloaded]
			...
			--skip-downloaded : If this option is activated, packages already downloader will not be downloaded
			  again (if you have stopped your execution and the process has not finished yet).
...
web page  --> http://deb-downloader.berlios.de/lists.php?lang=en .
